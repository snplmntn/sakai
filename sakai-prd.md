...
Recommended model strategy for the MVP:

- `gemini-2.5-flash` as the primary model for user-facing trip understanding, preference extraction, route explanation, and rider-facing MMDA summaries
- `gemini-2.5-flash-lite` as an optional lower-cost model for background normalization tasks such as MMDA alert field extraction, deduping support, or bulk reprocessing
- `gemini-2.5-pro` as an optional fallback or evaluation model for ambiguous requests, prompt testing, and quality benchmarking, not as the default production model for the MVP

Implementation guardrails:

- Sakai must not use the LLM to invent route options, fare amounts, or stop sequences
- Route generation, leg composition, fare calculation, and ranking logic should remain deterministic and data-backed
- The AI layer should be used to understand rider intent, structure noisy MMDA updates, and explain already-derived route outputs in simpler language
